...
export function LinkCategoryFilter() {
  const { linkCategories, selectedCategory, setSelectedCategory, links, getParentCategories, getSubcategories } = useLinks();
  ...
  const getCategoryCount = (categoryId: string | null | 'uncategorized') => {
    ...
    // Count links in this category AND all its subcategories
    const subcats = getSubcategories(categoryId);
    const subcatIds = subcats.map(s => s.id);
    return links.filter(l => l.categoryId === categoryId || subcatIds.includes(l.categoryId || '')).length;
  };
