...
export function LinkCategoryFilter() {
  ...
  const getCategoryCount = (categoryId: string | null | 'uncategorized') => {
    ...
    // Count links in this category AND its subcategories
    const childIds = linkCategories.filter(c => c.parentId === categoryId).map(c => c.id);
    return links.filter(l => l.categoryId === categoryId || childIds.includes(l.categoryId || '')).length;
  };
  ...
  const nonEmptyParentCategories = parentCategories.filter(cat => {
    ...
    const children = linkCategories.filter(c => c.parentId === cat.id);
  ...
  const getChildCategories = (parentId: string) => {
    return linkCategories.filter(c => c.parentId === parentId && getDirectCount(c.id) > 0);
  };

  const renderCategoryButton = (category: LinkCategory, isChild = false) => {
